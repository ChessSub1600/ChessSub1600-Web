...
const { guardarRegistroEnSheets, guardarReseñaEnSheets } = require('./sheets');
...
app.post('/reseñas', async (req, res) => {
  ...
  try {
    await guardarReseñaEnSheets(req.body);
    res.json({ 
      status: 'OK', 
...
app.post('/formulario', async (req, res) => {
  ...
  try {
    ...
    await enviarCorreo(
      ...
      generarHTMLBienvenida(req.body)
    );

    res.json({ status: 'OK', message: 'Registro guardado y correo enviado con éxito!' });
  } catch (error) {
    console.error('❌ Error:', error);
    res.status(500).json({ status: 'ERROR', message: 'Hubo un problema al registrar el formulario.' });
  }
});
...
function generarHTMLBienvenida({ nombre, interes }) {
  return `
...
<img src="https://ChessSub1600.github.io/ChessSub1600-Web/assets/img/caballo-negro.png" alt="ChessSub1600 Banner" style="max-width: 200px; width: 100%; height: auto; display: block; margin: auto;">
    <div style="padding: 20px;">
      <h2 style="color: #16325c;">¡Bienvenido a ChessSub1600, ${nombre}! &#x2654;</h2>
      <p>Gracias por registrarte en nuestra comunidad de ajedrez. Estamos encantados de tenerte con nosotros.</p>
      <p>Desde hoy recibirás contenido exclusivo para mejorar tu nivel, especialmente en el área de <strong>${interes}</strong>.</p>
